    ...
    (mutationsList, observer) => {
      ...
      if (attributeCheckboxes.length > 0) {
        ...
        applyButton.hidden =
          ...
          !Array.from(attributeCheckboxes).some((checkbox) => checkbox.checked);
  ...
  if (attributeFilters) {
    applyButton.addEventListener('click', () => {
      ...
      attributeFilters.forEach((attributeFilter) => {
        // urlParams.set('query_type_condition', 'or');
...
function updateApplyButtonState(applyButton, checkboxes, attributeCheckboxes) {
  ...
  applyButton.hidden = !(isAnyCategoryChecked || isAnyAttributeChecked);
}

// Function to update the state of the Apply button
function updateResetButtonState(resetButton, checkboxes, attributeCheckboxes) {
  const isAnyCategoryChecked = Array.from(checkboxes).some(
    (checkbox) => checkbox.checked
  );
  const isAnyAttributeChecked = Array.from(attributeCheckboxes).some(
  ...
  resetButton.hidden = !(isAnyCategoryChecked || isAnyAttributeChecked);
}
