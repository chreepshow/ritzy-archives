			...
			<tbody>
				...
				foreach ( $memberships as $key => $membership_id ) {
					...
					<tr class="woocommerce-orders-table__row woocommerce-orders-table__row--status-<?php echo esc_attr( $membership_status ); ?> order">
						<?php foreach ( $instance->membership_tab_headers() as $column_id => $column_name ) : ?>
							<td class="testcolumn woocommerce-orders-table__cell woocommerce-orders-table__cell-<?php echo esc_attr( $column_id ); ?>" data-title="<?php echo esc_attr( $column_name ); ?>">
                                ...
                                <?php elseif ( 'members-name' === $column_id ) : ?>
                                    <?php
                                    if ( ! empty( $membership_plan['post_title'] ) ) {
                                        /* translators: 1: formatted order total 2: total order items */
                                        echo sprintf( ' %s ', esc_html( $membership_plan['post_title'] ) );
                                    }
                                    ?>
